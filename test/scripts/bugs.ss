...
(module bugs (lib "halyard.ss" "halyard")
  ...
  (card /bugs/bug-f12767-measure-text-space-before-newline
    ...
    (def (text-tester at style name)
      ;; Draw the text in an element.
      (define t (new-text at style (.s) :name name))
      ;; Draw the text without the extra space (" ") using the same bounds.
      (draw-text (offset-rect (t .shape) at)
